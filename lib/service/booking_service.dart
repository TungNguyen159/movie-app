...
import 'package:movie_app/models/month_stat.dart';
...
class BookingService {
  ...
  Future<Booking?> getbookingbyid(String bookingid) async {
    final response = await supabase
        .from("bookings")
        .select("*,showtimes(*),user(name,email)")
        .eq("id", bookingid)
        .maybeSingle();
...
// update booking reviewed
  ...
  Future<List<Map<String, dynamic>>> getStatisticmovie() async {
    ...
    Map<int, Map<String, dynamic>> revenueMap = {};

    for (var row in response) {
      final movieId = row["showtimes"]["movie_id"] ?? 0;
